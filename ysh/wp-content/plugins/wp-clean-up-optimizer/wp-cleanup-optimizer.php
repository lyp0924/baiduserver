...
if ( ! defined( 'CLEAN_UP_OPTIMIZER_VERSION_NUMBER' ) ) {
	define( 'CLEAN_UP_OPTIMIZER_VERSION_NUMBER', '4.0.23' );
}
...
if ( $clean_up_optimizer_version_number >= '4.0.2' ) {
	...
	if ( ! function_exists( 'user_login_status_clean_up_optimizer' ) ) {
		...
		function user_login_status_clean_up_optimizer( $username, $password ) {
			...
			$ip_address = '::1' == $ip ? sprintf( '%u', ip2long( '127.0.0.1' ) ) : sprintf( '%u', ip2long( $ip ) );// WPCS: Loose Comparison ok.
		 ...
		 function get_ip_location_clean_up_optimizer( $ip_address ) {
 			...
 			if ( '' != $ip_location_meta_value ) {
				...
				if ( '' != $ip_location_array ) {
	 				...
	 				$wpdb->insert( clean_up_optimizer_ip_locations(), $ip_location_array_data );
				}
	...
	if ( ! function_exists( 'user_functions_clean_up_optimizer' ) ) {
		...
		function user_functions_clean_up_optimizer() {
			...
			$meta_values           = $wpdb->get_results(
				$wpdb->prepare(
					'SELECT meta_key,meta_value FROM ' . $wpdb->prefix . 'clean_up_optimizer_meta WHERE meta_key IN(%s,%s)', 'error_message', 'other_settings'
				)
			);// WPCS: db call ok, no-cache ok.
			$meta_values_ip_blocks = $wpdb->get_results(
				$wpdb->prepare(
					'SELECT meta_key,meta_value FROM ' . $wpdb->prefix . 'clean_up_optimizer_meta WHERE meta_key IN(%s,%s,%s)', 'block_ip_address', 'block_ip_range', 'country_blocks'
				)
			);// WPCS: db call ok, no-cache ok.
			...
			$ip_address = '::1' == get_ip_address_clean_up_optimizer() ? sprintf( '%u', ip2long( '127.0.0.1' ) ) : sprintf( '%u', ip2long( get_ip_address_clean_up_optimizer() ) );// WPCS Loose comparison ok.
			$location   = get_ip_location_clean_up_optimizer( long2ip_clean_up_optimizer( $ip_address ) );
	...
	function clean_up_optimizer_action_links( $plugin_link ) {
		$plugin_link[] = '<a href="http://beta.tech-banker.com/products/clean-up-optimizer/" style="color: red; font-weight: bold;" target="_blank">Go Pro!</a>';
		return $plugin_link;
	}
